...
void TuringMachine::InputCleanBox() {
    ...
    static set<unsigned int> finais;
    ...
    ImGui::Begin("Papadimitriou - joilnen", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
    ...
    for_each(begin(estados), end(estados), [](auto &a) {
        ...
        if (ImGui::Button(ss.str().c_str())) {
            if (finais.find(a) == end(finais)) finais.insert(a);
            else finais.erase(a);
        }
        ImGui::SameLine();
    });
    ...
    for_each(begin(finais), end(finais), [](auto &a) {
        stringstream ss;
        ss << "e" << a;
        ImGui::Text(ss.str().c_str(), ss.str().size()); ImGui::SameLine();
    });
    ImGui::Text("finais");
    ...
    for_each(begin(transicoes), end(transicoes), [](auto &a) {
        ...
        ss.clear(); ss.flush(); ss.str(std::string());
        ss << ":" << a.second.second;
        ImGui::Text(ss.str().c_str(), ss.str().size());
    });

    if (ImGui::Button("Checa Consistência"))
    {
        TempData t;
        for (int count = 0; entradaFita[count]; count++)
            t.fita.push_back(entradaFita[count]);
        for (int count = 0; entradaAlfabeto[count]; count++)
            t.alfabeto.push_back(entradaAlfabeto[count]);
        t.transicoes = &transicoes;
        t.simbIncial = 0x01;
        t.estados = &estados;
        t.maximoEstado = maximoEstado;
        t.finais = &finais;
    }
